...
public getMeshResolution(): 7 | 9 | 12 {
  ...
  public getColorRanges(variable: OceanVariable = this.activeVariable): ColorRange[] {
    return (this.colorRanges[variable] || DEFAULT_COLOR_RANGES[variable] || []).map((r) => ({ ...r }));
  }

  public setColorRanges(variable: OceanVariable, ranges: ColorRange[]): void {
    const validation = validateColorRanges(ranges);
  ...
  public getInterpolatedColorForVariable(variable: OceanVariable, value: number): string {
    const ranges = this.colorRanges[variable] || DEFAULT_COLOR_RANGES[variable] || [];
    return getInterpolatedColorForValue(value, ranges);
  }

  public getColorForActiveVariable(value: number): string {
    return this.getInterpolatedColorForVariable(this.activeVariable, value);
  }

  public getCesiumColorForVariable(variable: OceanVariable, value: number, alpha = 1): Cesium.Color {
    const ranges = this.colorRanges[variable] || DEFAULT_COLOR_RANGES[variable] || [];
    return cesiumColorFromHex(getInterpolatedColorForValue(value, ranges), alpha);
  }
  ...
  public getSnapshot(): OceanStateSnapshot {
    return {
      ...
      colorRanges: {
        ...
        oxygen: this.getColorRanges('oxygen'),
      },
    };
  }
