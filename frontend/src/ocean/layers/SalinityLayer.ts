...
export class SalinityLayer {
  ...
  private renderCanvas(): void {
    ...
    for (let y = 0; y < this.resolution; y++) {
      ...
      for (let x = 0; x < this.resolution; x++) {
        ...
        let r = Math.round(norm * 180 + (1.0 - norm) * 0);
        let g = Math.round((1.0 - norm) * 220 + norm * 20);
  ...
  private attachImageryLayer(): void {
    try {
      ...
      this.imageryLayer.alpha = 0.68;
      this.imageryLayer.show = this.active;
    } catch (err) {
      console.warn('SalinityLayer attach exception:', err);
    }
  }
  ...
  public update(): void {
    ...
    try {
      ...
      console.warn('SalinityLayer update exception:', err);
    }
  }
