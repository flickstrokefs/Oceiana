...
export const getColorForValue = (
  ...
  const valid = sortRanges(ranges).filter((r) => Number.isFinite(r.min) && Number.isFinite(r.max) && r.min < r.max);
  if (!valid.length || !Number.isFinite(value)) return normalizeHex(fallbackColor);

  // Each range color is an anchor at its minimum. The final maximum is also
  // an anchor at the final color. This produces a continuous scalar ramp.
  const anchors = valid.map((r) => ({ value: r.min, color: normalizeHex(r.color) }));
  anchors.push({ value: valid[valid.length - 1].max, color: normalizeHex(valid[valid.length - 1].color) });
  ...
  for (let i = 0; i < anchors.length - 1; i++) {
    ...
    if (value >= a.value && value <= b.value) {
      const t = (value - a.value) / Math.max(b.value - a.value, Number.EPSILON);
...
export const DEFAULT_COLOR_RANGES: Record<OceanVariable, ColorRange[]> = {
  temperature: [
    make('temperature-0', 2, 8, '#0022ff'), make('temperature-1', 8, 14, '#00d4ff'),
    make('temperature-2', 14, 20, '#00ff9d'), make('temperature-3', 20, 26, '#ffd000'),
    make('temperature-4', 26, 32, '#ff2200'),
  ],
  salinity: [
    make('salinity-0', 32, 33.5, '#2d5e94'), make('salinity-1', 33.5, 35, '#38bdf8'),
    make('salinity-2', 35, 36.5, '#facc15'), make('salinity-3', 36.5, 38, '#ef4444'),
  ],
  current: [
    make('current-0', 0, 0.75, '#161920'), make('current-1', 0.75, 1.5, '#2d5e94'),
    make('current-2', 1.5, 2.25, '#38bdf8'), make('current-3', 2.25, 3, '#f0f2f6'),
  ],
  chlorophyll: [
    make('chlorophyll-0', 0, 1.5, '#001f3f'), make('chlorophyll-1', 1.5, 3, '#0074D9'),
    make('chlorophyll-2', 3, 4.5, '#2ECC40'), make('chlorophyll-3', 4.5, 6, '#FFDC00'),
  ],
};
